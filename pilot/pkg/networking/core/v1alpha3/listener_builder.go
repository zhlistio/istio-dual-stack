...
func (lb *ListenerBuilder) aggregateVirtualInboundListener(passthroughInspectors map[int]enabledInspector) *ListenerBuilder {
	...
	for i := 0; i < len(lb.virtualInboundListener); i++ {
		...
		if lb.node.GetInterceptionMode() == model.InterceptionTproxy {
			lb.virtualInboundListener[i].ListenerFilters =
				append(lb.virtualInboundListener[i].ListenerFilters, xdsfilters.OriginalSrc)
		}
		...
		lb.virtualInboundListener[i].FilterChains =
			append(lb.virtualInboundListener[i].FilterChains, filterChains...)

		tlsInspectors := mergeInspectors(inspectors, passthroughInspectors)
		if needsTLS(tlsInspectors) {
			lb.virtualInboundListener[i].ListenerFilters =
				append(lb.virtualInboundListener[i].ListenerFilters, buildTLSInspector(tlsInspectors))
		}
		...
		if features.EnableProtocolSniffingForInbound {
			lb.virtualInboundListener[i].ListenerFilters =
				append(lb.virtualInboundListener[i].ListenerFilters, buildHTTPInspector(inspectors))
		}
...
func (lb *ListenerBuilder) buildVirtualInboundListener(configgen *ConfigGeneratorImpl) *ListenerBuilder {
	...
	for _, wildcard := range wildCards {
		...
		for _, aFilterChain := range filterChains {
			if aFilterChain.Name == model.VirtualInboundBlackholeFilterChainName || aFilterChain.Name == model.VirtualInboundCatchAllHTTPFilterChainName ||
...
func (configgen *ConfigGeneratorImpl) buildInboundFilterchains(in *plugin.InputParams, listenerOpts buildListenerOpts,
	...
	for _, opt := range newOpts {
		...
		if dualIpv6 && clusterName == util.InboundPassthroughClusterIpv6 {
			filterName += "6"
		} else if opt.fc.ListenerProtocol == istionetworking.ListenerProtocolHTTP {
