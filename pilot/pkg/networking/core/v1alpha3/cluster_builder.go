...
func (cb *ClusterBuilder) applyDestinationRule(mc *MutableCluster, clusterMode ClusterMode, service *model.Service,
	port *model.Port, proxyNetworkView map[network.ID]bool, destRule *config.Config, 
	serviceAccounts []string, traffic model.TrafficDirection) []*cluster.Cluster {
...
func (cb *ClusterBuilder) buildDefaultCluster(name string, discoveryType cluster.Cluster_DiscoveryType,
	...
	if direction == model.TrafficDirectionInbound || direction == model.TrafficDirectionInbound6 {
